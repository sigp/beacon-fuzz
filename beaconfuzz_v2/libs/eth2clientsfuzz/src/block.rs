...
pub fn run_block(beacon_blob: &[u8], data: &[u8], debug: bool) {
    ...
    if let Ok(att) = lighthouse::ssz_block(&data) {
        ...
        if let Ok(post) = lighthouse::process_block(beacon_clone, att.clone()) {
            ...
            // call prysm
            let res = prysm::process_block(&beacon_blob, &data, &post.as_ssz_bytes());
            ...
            // call nimbus
            let res = nimbus::process_block(&state.clone(), &att, &post.as_ssz_bytes());
            ...
            // call teku
            let res = teku::process_block(&state.clone(), &att, &post.as_ssz_bytes());
            ...
            let res = prysm::process_block(&beacon_blob, &data, &beacon_blob.clone());
            ...
            let res = nimbus::process_block(&state.clone(), &att, &beacon_blob.clone());
            ...
            let res = teku::process_block(&state.clone(), &att, &beacon_blob.clone());
