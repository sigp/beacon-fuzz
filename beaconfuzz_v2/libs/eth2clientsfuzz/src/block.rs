...
pub fn run_block(beacon_blob: &[u8], data: &[u8], debug: bool) {
    ...
    if let Ok(cont) = lighthouse::ssz_block(&data) {
        if debug {
        ...
        if let Ok(post) = lighthouse::process_block(beacon_clone, cont.clone()) {
            if debug {
                println!("[LIGHTHOUSE] Processing {}", true);
            }

            // call prysm
            let res = prysm::process_block(
