...
pub fn run_block_header(beacon_blob: &[u8], data: &[u8], debug: bool) {
    ...
    if let Ok(att) = lighthouse::ssz_block_header(&data) {
        ...
        if let Ok(post) = lighthouse::process_block_header(beacon_clone, att.clone()) {
            ...
            // call teku
            let res = teku::process_block_header(&state.clone(), &att, &post.as_ssz_bytes());
            ...
            let res = teku::process_block_header(&state.clone(), &att, &beacon_blob.clone());
