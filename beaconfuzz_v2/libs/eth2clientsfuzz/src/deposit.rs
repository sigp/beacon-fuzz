...
pub fn run_deposit(beacon_blob: &[u8], data: &[u8], debug: bool) {
    ...
    if let Ok(att) = lighthouse::ssz_deposit(&data) {
        ...
        if let Ok(post) = lighthouse::process_deposit(beacon_clone, att.clone()) {
            ...
            // call prysm
            let res = prysm::process_deposit(&beacon_blob, &data, &post.as_ssz_bytes());
            ...
            // call nimbus
            let res = nimbus::process_deposit(&state.clone(), &att, &post.as_ssz_bytes());
            ...
            let res = prysm::process_deposit(&beacon_blob, &data, &beacon_blob.clone());
            ...
            let res = nimbus::process_deposit(&state.clone(), &att, &beacon_blob.clone());
    ...
    // Data is an invalid SSZ container
        ...
        // Verify that prysm give same result than lighthouse
        let res = prysm::ssz_deposit(&data);
