...
pub fn run_voluntary_exit(beacon_blob: &[u8], data: &[u8], debug: bool) {
    ...
    if let Ok(att) = lighthouse::ssz_voluntary_exit(&data) {
        ...
        if let Ok(post) = lighthouse::process_voluntary_exit(beacon_clone, att.clone()) {
            ...
            // call prysm
            let res = prysm::process_voluntary_exit(&beacon_blob, &data, &post.as_ssz_bytes());
            ...
            // call nimbus
            let res = nimbus::process_voluntary_exit(&state.clone(), &att, &post.as_ssz_bytes());
            ...
            // call teku
            let res = teku::process_voluntary_exit(&state.clone(), &att, &post.as_ssz_bytes());
            ...
            let res = nimbus::process_voluntary_exit(&state.clone(), &att, &beacon_blob.clone());
            ...
            let res = teku::process_voluntary_exit(&state.clone(), &att, &beacon_blob.clone());
