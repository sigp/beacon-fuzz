...
pub struct AttesterSlashingTestCase<T: EthSpec> {
...
impl<T: EthSpec> AttesterSlashingTestCase<T> {
    ...
    pub fn process_attester_slashing(mut self) -> Result<BeaconState<T>, BlockProcessingError> {
        let spec = T::default_spec();

        let state = &mut self.pre;
